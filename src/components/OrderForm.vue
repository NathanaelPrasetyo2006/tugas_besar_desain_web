<template>
  <form
    ...
    class="bg-white rounded-xl overflow-hidden space-y-6 shadow-lg p-10 max-w-3xl w-full transition"
  >
    <div>
      ...
      <input
        v-model="form.nama"
        type="text"
        class="w-full border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 scroll-mt-24"
    ...
    <div>
      <label class="block font-semibold mb-2">Nomor Kontak</label>
      <input
    ...
    <div>
      <h2 class="font-semibold text-lg mb-2">Produk Dipesan</h2>
      <div
        v-for="(item, index) in form.produk"
        :key="index"
        class="border p-4 rounded-lg mb-4 bg-gray-50"
      >
        <label class="block font-semibold mb-2">Jenis Roti</label>
        <input
          v-model="item.jenis"
          type="text"
          class="w-full border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 scroll-mt-24"
        ...
        <label class="block font-semibold mb-2">Jumlah Roti</label>
        <input
        ...
        <button
          type="button"
          class="text-red-500 mt-2 underline hover:text-red-700 transition"
    ...
    <div>
      <label class="block font-semibold mb-2">Tanggal Pengiriman/Pengambilan</label>
      <input
        v-model="form.tanggal"
        type="date"
        class="w-full border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 scroll-mt-24"
        placeholder="Masukkan tanggal pengiriman/pengambilan"
        required
      />
    </div>

    <div>
      <label class="block font-semibold mb-2">Alamat Lengkap (Jika Diantar)</label>
      <input
        v-model="form.alamat"
        type="text"
    ...
    <div>
      <label class="block font-semibold mb-2"
        >Catatan Khusus (misalnya: request dekorasi/topping)</label
      >
      <textarea
